...
def plot_eigenvector(
    ...
    def create_axe(
        ...
        pot = potential.V.sel(initial_potential_sel).squeeze().real
    ...
    def update(**kwargs):
        ...
        for i in range(n_rows):
            for j in range(n_cols):
                ...
                new_potentials += [potentials[i][j].V.sel(potential_sel, method = 'nearest').squeeze().real]
...
def plot_cuts(
    ...
    if len(groupby)>0:
        for i, b in enumerate(eigva_stack.stacked):
            line = ax.plot(eigva.coords[dim], initial_band.sel(stacked = b), **list_linekw[i%len(list_linekw)])
            lines += line
    else:
...
def energy_levels(
    ...
    ymin:float = None,
    ymax:float = None,
    frac:float = 0.05)->tuple[Figure, Axes]:
...
def dashboard(
    ...
    def create_axe(
        ...
        field = plot.sel(initial_field_sel)
        pot = potential.V.sel(initial_potential_sel).real
        ...
        for i in range(n_rows):
            for j in range(n_cols):
                if eigveplots[i][j] is not None:
                    ...
                    new_potentials += [potential.V.sel(potential_sel, method = 'nearest').real]
